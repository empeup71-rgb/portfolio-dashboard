...
const ReportTab = ({ holdings, T }) => {
  ...
  const tplp = tc>0?tpl/tc*100:0;
  const tdiv = holdings.reduce((s,h)=>s+h.qty*(h.div||0),0);
  const avgB = holdings.reduce((s,h)=>s+(h.beta||1),0)/Math.max(holdings.length,1);
  ...
  const generateReport = () => {
    ...
    setTimeout(()=>{
      ...
      setGenerated(true);
      // Build report HTML
      const reportDate = new Date().toLocaleDateString("en",{year:"numeric",month:"long",day:"numeric"});
      const html = `
<!DOCTYPE html>
<html>
<head>
  ...
  <style>
    * { margin:0; padding:0; box-sizing:border-box; }
    body { font-family: 'Arial', sans-serif; background:#020D18; color:#E2EAF4; padding:40px; }
    .header { border-bottom:2px solid #C9A84C; padding-bottom:24px; margin-bottom:32px; display:flex; justify-content:space-between; align-items:flex-end; }
    .logo { font-size:28px; font-weight:900; background:linear-gradient(90deg,#C9A84C,#F0C060); -webkit-background-clip:text; -webkit-text-fill-color:transparent; }
    .subtitle { font-size:11px; color:#3D5066; font-family:monospace; letter-spacing:2px; margin-top:4px; }
    .date { font-size:12px; color:#3D5066; font-family:monospace; text-align:right; }
    .section { margin-bottom:32px; }
    .section-title { font-size:14px; font-weight:700; color:#C9A84C; letter-spacing:1px; text-transform:uppercase; margin-bottom:16px; border-left:4px solid #C9A84C; padding-left:12px; }
    .kpi-grid { display:grid; grid-template-columns:repeat(4,1fr); gap:12px; margin-bottom:24px; }
    .kpi { background:rgba(201,168,76,0.08); border:1px solid rgba(201,168,76,0.2); border-radius:10px; padding:14px 16px; }
    .kpi-label { font-size:8px; color:#3D5066; letter-spacing:2px; text-transform:uppercase; margin-bottom:6px; font-family:monospace; }
    .kpi-value { font-size:20px; font-weight:800; color:#C9A84C; }
    .kpi-sub { font-size:10px; color:#3D5066; margin-top:4px; font-family:monospace; }
    table { width:100%; border-collapse:collapse; font-size:12px; }
    th { text-align:left; padding:8px 12px; color:#3D5066; font-family:monospace; font-size:9px; letter-spacing:1px; border-bottom:1px solid rgba(255,255,255,0.08); }
    td { padding:10px 12px; border-bottom:1px solid rgba(255,255,255,0.04); }
    .symbol { font-weight:700; color:#C9A84C; font-family:monospace; }
    .positive { color:#00C896; font-weight:600; }
    .negative { color:#FF4C4C; font-weight:600; }
    .footer { margin-top:48px; padding-top:16px; border-top:1px solid rgba(255,255,255,0.06); text-align:center; font-size:9px; color:#3D5066; font-family:monospace; letter-spacing:2px; }
    .chip { display:inline-block; padding:2px 8px; border-radius:4px; font-size:9px; font-weight:700; font-family:monospace; }
    .chip-stock  { background:rgba(30,144,255,0.15); color:#1E90FF; }
    .chip-etf    { background:rgba(123,104,238,0.15); color:#7B68EE; }
    .chip-crypto { background:rgba(255,179,0,0.15); color:#FFB300; }
    .chip-bond   { background:rgba(0,200,150,0.15); color:#00C896; }
  </style>
</head>
<body>
  <div class="header">
    <div>
      ...
      <div class="subtitle">INSTITUTIONAL PORTFOLIO REPORT · v3.0</div>
    </div>
    <div class="date">
      <div style="font-size:18px;font-weight:700;color:#E2EAF4">${reportDate}</div>
      <div style="margin-top:4px">Generated by REPORT Bot</div>
    </div>
  </div>
  ...
  <div class="section">
    ...
    <div class="kpi-grid">
      <div class="kpi"><div class="kpi-label">Total Value</div><div class="kpi-value">$${Math.round(tv).toLocaleString()}</div><div class="kpi-sub">${holdings.length} positions</div></div>
      <div class="kpi"><div class="kpi-label">Total P&L</div><div class="kpi-value" style="color:${tpl>=0?"#00C896":"#FF4C4C"}">${tpl>=0?"+":"-"}$${Math.abs(Math.round(tpl)).toLocaleString()}</div><div class="kpi-sub">${tplp>=0?"+":""}${tplp.toFixed(2)}% return</div></div>
      <div class="kpi"><div class="kpi-label">Annual Dividends</div><div class="kpi-value" style="color:#00C853">$${Math.round(tdiv).toLocaleString()}</div><div class="kpi-sub">Yield: ${(tdiv/tv*100).toFixed(2)}%</div></div>
      <div class="kpi"><div class="kpi-label">Avg Beta</div><div class="kpi-value" style="color:#7B68EE">${avgB.toFixed(2)}</div><div class="kpi-sub">Market sensitivity</div></div>
    </div>
  </div>
  ...
  <div class="section">
    ...
    <table>
      <thead><tr>
        <th>Symbol</th><th>Name</th><th>Broker</th><th>Portfolio</th><th>Type</th>
        <th>Qty</th><th>Price</th><th>Value</th><th>P&L $</th><th>P&L %</th>
      </tr></thead>
      <tbody>
        ${[...holdings].sort((a,b)=>V(b)-V(a)).map(h=>`
        <tr>
          <td class="symbol">${h.symbol}</td>
          <td style="color:#3D5066;font-size:11px">${h.name}</td>
          <td style="color:#3D5066;font-size:11px">${h.broker.split(" ")[0]}</td>
          <td style="color:#3D5066;font-size:11px">${h.portfolio||"—"}</td>
          <td><span class="chip chip-${h.type.toLowerCase()}">${h.type}</span></td>
          <td style="font-family:monospace">${h.qty}</td>
          <td style="font-family:monospace">$${h.price.toLocaleString()}</td>
          <td style="font-family:monospace;font-weight:700">$${Math.round(V(h)).toLocaleString()}</td>
          <td class="${PL(h)>=0?"positive":"negative"}">${PL(h)>=0?"+":"-"}$${Math.abs(Math.round(PL(h))).toLocaleString()}</td>
          <td class="${PLP(h)>=0?"positive":"negative"}">${PLP(h)>=0?"+":""}${PLP(h).toFixed(2)}%</td>
        </tr>`).join("")}
      </tbody>
    </table>
  </div>
  ...
  <div class="section">
    ...
    <div class="kpi-grid">
      <div class="kpi"><div class="kpi-label">Sharpe Ratio</div><div class="kpi-value" style="color:#00C896">1.84</div><div class="kpi-sub">Risk-adj return</div></div>
      <div class="kpi"><div class="kpi-label">Sortino Ratio</div><div class="kpi-value" style="color:#1E90FF">2.31</div><div class="kpi-sub">Downside risk-adj</div></div>
      <div class="kpi"><div class="kpi-label">VaR (95%)</div><div class="kpi-value" style="color:#FF4C4C">-1.8%</div><div class="kpi-sub">Daily at-risk</div></div>
      <div class="kpi"><div class="kpi-label">Portfolio Beta</div><div class="kpi-value" style="color:#7B68EE">${avgB.toFixed(2)}</div><div class="kpi-sub">Market sensitivity</div></div>
    </div>
  </div>

  <div class="footer">
    PORTFOLIO COMMAND CENTER v3.0 · INSTITUTIONAL GRADE · POWERED BY 15 AI AGENTS · CONFIDENTIAL
  </div>
</body>
</html>`;

      // Download as HTML (opens in browser, can print to PDF)
  ...
  const REPORT_TYPES = [
    {id:"full",     label:"Full Report",         icon:"📄", desc:"Complete analysis — all holdings, risk metrics, performance"},
    {id:"summary",  label:"Executive Summary",   icon:"📊", desc:"1-page overview — key KPIs and top positions"},
    {id:"tax",      label:"Tax Report",          icon:"🧾", desc:"Realized gains/losses — for tax filing purposes"},
    {id:"dividend", label:"Dividend Report",     icon:"💰", desc:"Income analysis — yield, YoC, projected income"},
  ];

  return (
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
        <KPI label="Holdings"       value={holdings.length}                                    color={BRAND.gold}   T={T}/>
        <KPI label="Portfolio Value"value={`$${Math.round(tv).toLocaleString()}`}             color={BRAND.teal}   T={T}/>
        <KPI label="Total P&L"      value={`${tpl>=0?"+":"-"}$${Math.abs(Math.round(tpl)).toLocaleString()}`} color={tpl>=0?BRAND.teal:BRAND.red} T={T}/>
        <KPI label="Report Date"    value={new Date().toLocaleDateString()}                    color={BRAND.blue}   T={T}/>
      </div>

      {/* Report Type Selection */}
      <Card T={T}>
        <STN title="Select Report Type" sub="Choose what to include in your report" color={BRAND.gold} T={T}/>
        <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:10,marginBottom:20}}>
          {REPORT_TYPES.map(r=>(
            <div key={r.id} onClick={()=>setReportType(r.id)} style={{
              padding:"14px 18px",borderRadius:12,cursor:"pointer",
              background:reportType===r.id?`${BRAND.gold}12`:T.surface,
              border:`1px solid ${reportType===r.id?BRAND.gold+"55":T.border}`,
              transition:"all 0.2s",
            }}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
                <span style={{fontSize:20}}>{r.icon}</span>
                <span style={{fontFamily:BRAND.display,fontWeight:700,fontSize:13,color:reportType===r.id?BRAND.gold:T.text}}>{r.label}</span>
                {reportType===r.id&&<Chip label="Selected" color={BRAND.gold} size={8}/>}
              </div>
              <div style={{fontSize:11,fontFamily:BRAND.mono,color:T.muted,lineHeight:1.5}}>{r.desc}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          <button onClick={generateReport} disabled={generating} style={{
            ...
            background:generating?T.surface:`linear-gradient(135deg,${BRAND.gold},${BRAND.gold2})`,
            color:generating?T.muted:BRAND.bg,
            fontFamily:BRAND.display,fontSize:14,fontWeight:800,
            boxShadow:generating?"none":`0 0 24px ${BRAND.gold}44`,
            transition:"all 0.3s",
            display:"flex",alignItems:"center",gap:10,
          }}>
            {generating?(
              <>
                <div style={{width:16,height:16,borderRadius:"50%",border:`2px solid ${T.muted}`,borderTopColor:BRAND.gold,animation:"spin 0.8s linear infinite"}}/>
                Generating Report...
              </>
            ):"📄 Generate & Download Report"}
          </button>
          {generated&&(
            <div style={{display:"flex",alignItems:"center",gap:6,padding:"8px 14px",background:`${BRAND.teal}10`,border:`1px solid ${BRAND.teal}33`,borderRadius:9}}>
              <span style={{fontSize:14}}>✅</span>
              <span style={{fontFamily:BRAND.mono,fontSize:11,color:BRAND.teal}}>Report downloaded! Open in browser → Print → Save as PDF</span>
            </div>
          )}
        </div>
      </Card>
      ...
      {/* Preview */}
      <Card T={T} glow={BRAND.blue}>
        <STN title="Report Preview" sub="What will be included in your report" color={BRAND.blue} T={T}/>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
          <div>
            <div style={{fontSize:10,fontFamily:BRAND.mono,color:T.muted,letterSpacing:2,marginBottom:10}}>INCLUDED SECTIONS</div>
            {[
              {icon:"📊",label:"Portfolio Summary — 4 KPIs"},
              {icon:"📋",label:`Holdings Table — ${holdings.length} positions`},
              {icon:"🛡",label:"Risk Metrics — Sharpe, Sortino, VaR, Beta"},
              {icon:"💰",label:"Dividend Analysis"},
              {icon:"🤖",label:"AI Agent Summary"},
              {icon:"📅",label:`Report Date: ${new Date().toLocaleDateString()}`},
            ].map((s,i)=>(
              <div key={i} style={{display:"flex",alignItems:"center",gap:8,marginBottom:7,padding:"7px 11px",background:T.surface,borderRadius:7}}>
                <span style={{fontSize:14}}>{s.icon}</span>
                <span style={{fontSize:11,fontFamily:BRAND.mono,color:T.text}}>{s.label}</span>
              </div>
            ))}
          </div>
          <div>
            <div style={{fontSize:10,fontFamily:BRAND.mono,color:T.muted,letterSpacing:2,marginBottom:10}}>TOP HOLDINGS PREVIEW</div>
            {[...holdings].sort((a,b)=>V(b)-V(a)).slice(0,5).map((h,i)=>(
              <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6,padding:"7px 11px",background:T.surface,borderRadius:7}}>
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  <span style={{fontFamily:BRAND.mono,fontWeight:700,color:BRAND.gold,fontSize:12}}>{h.symbol}</span>
                  <Chip label={h.type} color={TYPE_COLOR[h.type]||BRAND.gold} size={8}/>
                </div>
                <div style={{textAlign:"right"}}>
                  <div style={{fontFamily:BRAND.mono,fontWeight:700,fontSize:12,color:T.text}}>${Math.round(V(h)).toLocaleString()}</div>
                  <div style={{fontFamily:BRAND.mono,fontSize:10,color:PLP(h)>=0?BRAND.teal:BRAND.red}}>{PLP(h)>=0?"+":""}{PLP(h).toFixed(2)}%</div>
                </div>
              </div>
            ))}
          </div>
...
const AnalysisPanel = ({ holdings, T, activeTab, botMem, setBotMem }) => {
  return (
    <div style={{animation:"fadeIn 0.25s ease"}}>
      ...
      {activeTab==="report"  && <ReportTab        holdings={holdings} T={T}/>}
    </div>
  );
};
      ...
      <div style={{borderBottom:`1px solid ${BRAND.gold}14`,background:T.header,backdropFilter:"blur(24px)",position:"sticky",top:0,zIndex:100,boxShadow:T.mode==="dark"?"0 4px 32px rgba(0,0,0,0.5)":"0 4px 20px rgba(0,0,0,0.08)"}}>
        <div style={{padding:"0 24px"}}>
          ...
          <div style={{borderBottom:`1px solid ${T.border}`,paddingBottom:0}}>
            {/* Row 1 */}
            <div style={{display:"grid",gridTemplateColumns:"repeat(8,1fr)",gap:3,padding:"4px 0 3px 0"}}>
              {[
                ...
                {id:"report",    label:"Export",       icon:"📄", color:BRAND.teal},
              ].map(t=>{
